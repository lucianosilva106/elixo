...
import TransacoesPDF from '../Relatorios/Transacoes/index';
import './transac.css';
        ...
        .onSnapshot((doc) => {
          ...
          doc.forEach((item) => {
              minhasVendas.push({
                  ...
                  valorcomissao: item.data().valorcomissao
              })
          })
        ...
        .onSnapshot((doc) => {
          ...
          doc.forEach((item) => {
              minhasVendaprodutos.push({
                  ...
                  info: item.data().info,
                  valorunitario: item.data().valorunitario,
                  quantidade: item.data().quantidade,
                  valortotal: item.data().valortotal,
                  data: item.data().data,
                  formapagamento: item.data().formapagamento,
                  percentualcomissao: item.data().percentualcomissao,
                  valorcomissao: item.data().valorcomissao,
                  nomecomprador: item.data().nomecomprador,
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      sx={{height: 150, marginTop: 6}}>
        ...
        <button onClick={(e) => TransacoesPDF(vendaprodutos) }>Fecha Comissão/PDF</button>
      </Container>
      <Container fixed>
          ...
          <Table sx={{ minWidth: 650 }} size="small" aria-label="a dense table">
            <TableHead sx={{ color: 'primary' }}>
              <TableRow>
                ...
                <TableCell align="center">Valor Comissao</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {vendas.map((venda) => {
                return (
                  <TableRow key={venda.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                    ...
                    <TableCell align="center">{venda.nomecliente}</TableCell>
                    <TableCell align="center">{venda.valortotal}</TableCell>
                    <TableCell align="center">{venda.valorcomissao}</TableCell>
